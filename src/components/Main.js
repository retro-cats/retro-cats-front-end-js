...
const tabOptions = ["mint cats", "my cats", "search cats"];
...
export const Main = () => {
  ...
  const retroCatsAddress = isValidNetwork(networkId)
    ? networkMapping[networkId.toString()]["RetroCats"][0]
        ...
        isValidNetwork(networkId) ? (
          <Container>
            <TabContext value={selectedTab.toString()}>
              ...
              {tabOptions.map((tabOption) => {
                return (
                  <TabPanel className={classes.tabContent} value={tabOption} key={tabOption}>
                    {tabOption === "mint cats" ? (
                      <MintCats networkId={networkId} retroCatsAddress={retroCatsAddress} />
                    ) : tabOption === "my cats" ? (
                      <MyCats networkId={networkId} retroCatsAddress={retroCatsAddress} />
                    ) : (
                      <SearchCats networkId={networkId} retroCatsAddress={retroCatsAddress} />
                    )}
                  </TabPanel>
                );
              })}
